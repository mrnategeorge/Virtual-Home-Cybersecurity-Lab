...
### Example Topology (Mermaid Diagram)
```mermaid
graph TD
  Router --> Switch
  Switch --> DC[Windows Server 2025 - Domain Controller]
  Switch --> Email[Ubuntu Desktop - MailHog + Docker]
  Switch --> SecServer[Ubuntu Server - Wazuh SIEM/XDR]
  Switch --> Win11[Employee Workstation - Windows 11 + Wazuh]
  Switch --> UbuntuWS[Employee Workstation - Ubuntu + Wazuh]
  Switch --> SO[Security Onion]
  Switch --> Kali[Kali Linux - Attacker]
````
